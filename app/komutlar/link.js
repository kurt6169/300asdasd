...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`<a:ayar2:860652076656492544> Reklam Filtresini Ayarlamak İçin \`e+link aç\` | Kapatmak İstiyorsanız \`e+link kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`e+link aç\` | Kapatmak İstiyorsanız \`e+link kapat\` Yazabilirsiniz`)
